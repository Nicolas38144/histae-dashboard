...
const labels: Record<string, string> = {
  ...
  awaiting_continuation: 'Décision attendue', expired: 'Expiré', ended: 'Terminé',
};

export function StatusChip({ value }: { value: string }) {
  const color = value === 'active' || value === 'confirmed' || value === 'completed' || value === 'reviewed'
    ? 'success' : value === 'pending' || value === 'in_progress' || value === 'awaiting_continuation'
      ? 'warning' : value === 'banned' || value === 'rejected' ? 'error' : 'default';
