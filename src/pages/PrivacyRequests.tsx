    ...
    <>
      <PageHeader title="Demandes RGPD" description="Suivi des droits d’accès, d’effacement, de portabilité et de rectification." actions={
        <Box sx={{ display: 'flex', gap: 2 }}>
          <Button disabled={loading || saving} onClick={reload}>Actualiser</Button>
      ...
      {data && <Paper variant="outlined" sx={{ overflowX: 'auto' }}><Table size="small">
        ...
        <TableBody>{data.map((request) => <TableRow key={request.id}>
        ...
        </TableRow>)}{!data.length && <TableRow><TableCell colSpan={7} align="center" sx={{ py: 5 }}>Aucune demande dans cette file.</TableCell></TableRow>}</TableBody>
      </Table></Paper>}
